	...
	*/
	...
	// Looping Data
		while ( $aRow = $rResult->fetch_assoc() ) {
			$row = array();
			$btn = '<td><center>
			<a onClick="showDetail(\''.$aRow['id_penjualan'].'\')" data-toggle="tooltip" title="Details" class="btn  btn-sm btn-info btn-fill"> <i class="fa fa-th-list" aria-hidden="true"></i></a>
			<a onClick="deletePenjualan(\''.$aRow['id_penjualan'].'\')"   data-toggle="tooltip" title="Hapus" class="btn btn-sm btn-fill btn-danger"> <i class="fa fa-trash" aria-hidden="true"></i></a>
		</center></td>';
		$row = array($aRow['id_penjualan'], $aRow['tanggal'], $aRow['nama_anggota'], 'Rp. ' . number_format($aRow['total'], 0, ".", "."), $btn);
		$output['aaData'][] = $row;
	}
